...
动画层包含了游戏中所有元素的动画，碰撞检测和其它的游戏逻辑，也许叫它**游戏层(GamePlayLayer)**会更加贴切，你可以挑你喜欢的叫法。我们在这层中组织了我们游戏的关键部分。总的来说，我们将会设计游戏对象，等级**spawner**（也叫做等级管理器（level managers）），不同物体之间的碰撞检测还有胜负条件。所有的繁琐的事情都会在这一层。
